...
public class ApplicationView extends View implements ClickHandler {
	...
	public enum LinksGroup {
		HOME(constants.Home()), SIGN_IN(constants.SignIn()), CHANGE_PASS(
				constants.ChangePassword()), REGISTER(constants.Register()), EDIT_COLLABORATORS(
	...
	private class Links extends Composite {
		...
		/**
		 * {@link FlowPanel#clear()} also clears HTML content element so use
		 * this method to clear {@link #linksPanel} instance of
		 * {@link FlowPanel#clear()} .
		 */
		void clear() {
		 ...
		 * 
		 * @param keyword
		 *            the keyword that identifies the array of link widgets
		 * @param widgets
		 *            the array of link widgets
		 */
		 ...
		 * 
		 * @param keyword
		 *            the keyword that identifies the links to show
		 */
		...
		/**
		 * A helper method that adds the link widget along with a separator
		 * after it.
		 * 
		 * @param link
		 *            the link to add
		 ...
		 *            the link to add
		 * @param addSeparator
		 *            if true add a separator after the link
		 */
	...
	/**
	 * Returns a list of values created from a parameter in a history token, or
	 * an empty list.
	 * 
	 * @param historyToken
	 * @param paramName
	...
	public static void setAuthenticatedUser(User user) {
		...
		if (user != null) {
			...
			// Checks whether session is still valid in each 15 minutes
			// (900000ms)
	...
	public static void refreshSession() {
		if (sessionId != null) {
			Cookies.setCookie(SESSION_ID_NAME, getSessionId(),
	...
	/**
	 * Checks whether the current session id is still valid. If the user is
	 * logged again in other system, the session id of the older systems will be
	 * invalid.
	 */
	 ...
	 * check whether the session id is still valid. This checks is too ensure
	 * the user only logged on one system at a time because the current server
		...
		tabPanel.addSelectionHandler(new SelectionHandler<Integer>() {
			public void onSelection(SelectionEvent<Integer> event) {
				...
				String tabName = tabPanel.getTabBar().getTabHTML(selectedIndex)
						.toLowerCase();
				/**
				 * if the selectedWidget is a label which mean is it is from
				 * initial dummy label {@link ApplicationView#addTab(ViewInfo)}
				 * 
				 */
				if (selectedWidget instanceof Label) {
					String viewName = ((Label) selectedWidget).getText()
							.toLowerCase();
				...
				if (activeViewInfo != null
					...
					if (selectedWidget instanceof UploadView
							|| selectedWidget instanceof FormView
							|| selectedWidget instanceof SpeciesExplorerView) {
						activeViewInfo.getView().resetToDefaultState();
					}
					// { WD fix the resizing issue
					activeViewInfo.getView().onResize(
							new ResizeEvent(Window.getClientWidth(), Window
									.getClientHeight()) {
							});
	...
	/**
	 * Construct this view history token (i.e tab=occurrences) and concat it
	 * with active child view history token.
	 */
	@Override
	public String historyToken() {
		...
		historyToken += activeViewToken.equals("") ? ""
				: ("&" + activeViewToken);
		return historyToken;
	}
	 ...
	 * {@link ViewInfo}.
	 * 
	 * @param loadDefaultView
	 *            true if default child view should be loaded. Default child
	 *            view is OccurrenceView
	 */
	...
	public void onClick(ClickEvent event) {
		 ...
		 * else if (sender == changePassLink) { switchView(CHANGE_PASS); } else
		 * if (sender == editCollaboratorsLink) {
		 * switchView(EDIT_COLLABORATORS); }
		 */
	...
	/**
	 * Switches child view to the given view if the child view is not the same
	 * as active view nor active view is null. When a view is switch change the
	 * links to propriated links.
	 * 
	...
	protected void switchView(String view, boolean addHistory) {
		...
		if (activeViewInfo == null
			...
			// pour cacher les CustomPopupPanel (map et list view ) de
			// OccurrenceView
			...
			// on en profite pour ajouter l'occurenceView comme
			// OccurrenceSearchListener de speciesExplorerView
			ViewInfo spExpViewInfo = viewInfos.get(SPECIES_EXPLORER
					.toLowerCase());
			SpeciesExplorerView spExpView = (SpeciesExplorerView) spExpViewInfo
					.getView();
			spExpView
					.addOccurrenceSearchListener((OccurrenceSearchListener) occView);
			// on en profite pour ajouter l'occurenceView comme
						// OccurrenceSearchListener de homeView
			//RORO
			ViewInfo homeViewInfo = viewInfos.get(HOME
					.toLowerCase());
			HomeView homeView = (HomeView) homeViewInfo
					.getView();
			...
			// StatisticsTabView statsView = (StatisticsTabView)
			// statViewInfo.getView();
			...
			// EmailingTabView mailingView =
			// (EmailingTabView)mailingInfo.getView();
			...
			if (view.equalsIgnoreCase(HOME)) {
				...
				currentTabs.add(viewInfos.get(HOME.toLowerCase()));// RORO
				currentTabs.add(viewInfos.get(OCCURRENCES.toLowerCase()));
				currentTabs.add(viewInfos.get(SPECIES_EXPLORER.toLowerCase()));
				currentTabs.add(viewInfos.get(STATS_TAB.toLowerCase()));
				if (isSignedIn) {
	...
	private void addTab(ViewInfo viewInfo) {
		// the dummy label is to ensure the application does not load
		// unnecessary
		// view
	...
	private String getUrlToken(UrlParam urlParam) {
		...
		switch (urlParam) {
		case TAB:
			// RORO String tabView = OCCURRENCES.toLowerCase();//
			String tabView = HOME.toLowerCase();// RORO
			if (activeViewInfo != null) {
	...
	private void initLinks() {
		...
		links.putLinks(LinksGroup.HOME.toString() + true, new Widget[] {
				version, signOutLink, webPortalLink, issuesLink, helpLink,
				acknowledgmentLink, locale });
		// false is the key for unauthenticated user
		links.putLinks(LinksGroup.HOME.toString() + false, new Widget[] {
				version, signinLink, regLink, webPortalLink, issuesLink,
				helpLink, acknowledgmentLink, locale });
		links.putLinks(LinksGroup.SIGN_IN + "", new Widget[] { version,
				homeLink, regLink, webPortalLink, issuesLink, helpLink,
				acknowledgmentLink, locale });
		links.putLinks(LinksGroup.CHANGE_PASS + "", new Widget[] { version,
				homeLink, signOutLink, webPortalLink, issuesLink, helpLink,
				acknowledgmentLink, locale });
		links.putLinks(LinksGroup.REGISTER + "", new Widget[] { version,
				homeLink, signinLink, webPortalLink, issuesLink, helpLink,
				acknowledgmentLink, locale });
		links.putLinks(LinksGroup.EDIT_COLLABORATORS + "", new Widget[] {
				version, homeLink, signOutLink, webPortalLink, issuesLink,
				helpLink, acknowledgmentLink, locale });
		switch (currentState) {
		case SUPERADMIN:
		case ADMIN:
	...
	private void initViews() {
		ViewInfo occurrenveViewInfo = OccurrenceView.init(this,
		...
		ViewInfo changePassViewInfo = new FormViewInfo(
				constants.ChangePassword(), CHANGE_PASS, this,
				new ChangePasswordForm(constants.ChangePassword(),
						new FormListener() {
							public void onSubmitFailed(Throwable t) {
		...
		ViewInfo forgetPassViewInfo = new FormViewInfo(
				constants.ForgotPassword(), FORGET_PASS, this,
				new ForgotPasswordForm(constants.RecoverPasswordMsg(),
						new FormListener() {
							public void onSubmitFailed(Throwable t) {
		...
		editCollaboratorsViewInfo = UserProfilesView.init(this,
				constants.UserProfiles(), USER_PROFILES);
		userManagementViewInfo = UserManagementView.init(this,
				constants.UserManagement(), USER_MANAGEMENT);
		ViewInfo speciesExplorerViewInfo = SpeciesExplorerView.init(this,
				constants.SpeciesExplorer(), SPECIES_EXPLORER);
		...
		ViewInfo mailingViewInfo = MailingTabView.init(this,
				constants.mailing(), MAILING);
		addViewInfo(occurrenveViewInfo);
		addViewInfo(speciesExplorerViewInfo);
		addViewInfo(statsViewInfo);
		addViewInfo(homeViewInfo);// RORO
	...
	/**
	 * Signs out the user on the server, invalidates the session, updates
	 * content state, and shows the unauthenticated navigation links.
	 * 
	 * @param serverSignOut
	 *            true to sign out the user on server as well.
	 */
	private void signOutUser(boolean serverSignOut) {
		...
		// HACK: UserProfilesView maintains a final User reference that is set
		// when
		// initialized. Need to clobber that user when user logs out we
		// reinitializing the view.
		viewInfos.remove(editCollaboratorsViewInfo.getHisTokenName()
				.toLowerCase());
		editCollaboratorsViewInfo = UserProfilesView.init(this,
				constants.UserProfiles(), USER_PROFILES);
		addViewInfo(editCollaboratorsViewInfo);
