...
public class DetailView extends ComponentView implements OpenHandler<TreeItem>,
    SelectionHandler<TreeItem>, PageListener<Occurrence>, PageClickListener,
    DataRequestListener, OccurrencePageSizeChangeHandler {
  ...
  public static class FieldConstants {
    ...
    TIPS_MAP.put(ADJUSTED_COORDINATE_UNCERTAINTY_IN_METERS,
      ...
      TIPS_MAP.put(ETP_TOTAL1950, "Annual total evapotranspiration calculated by summing 12 monthly evapotranspiration rates - 1950");
      ...
      TIPS_MAP
          .put(MAX_TEMP2000, "Mean temperature of the hottest month - 2000");
      TIPS_MAP.put(MAX_TEMPFUTURE,
          "Mean temperature of the hottest month - Future");
      TIPS_MAP.put(MAXTEMP1950, "Mean temperature of the hottest month - 1950");
      ...
      TIPS_MAP
          .put(MIN_TEMP2000, "Mean temperature of the coldest month - 2000");
      TIPS_MAP.put(MIN_TEMPFUTURE,
          "Mean temperature of the coldest month - Future");
      TIPS_MAP
          .put(MIN_TEMP1950, "Mean temperature of the coldest month - 1950");
  ...
  /**
   * A editable widget where user input is validated based on Occurrence fields.
   * 
  ...
  abstract class FieldEditor extends Composite implements KeyUpHandler,
    ...
    /**
     * A CSS style for display field, value field that are not current editing.
     */
    ...
    /**
     * Initializes this widget with the given field name and the original field
     * value.
     * 
     * @param fieldName the name of this field
     * @param fieldValue the value of this field
     */
    public FieldEditor(String fieldName, String fieldValue) {
      ...
      input.addBlurHandler(new BlurHandler() {
        /**
         * When a user is clicked out site of the editing text box, enforces
         * enter key action.
         * 
         * @param sender text box widget
         */
     ...
     * 
     * @param changeListener {@link FieldChangeListener};
     */
    ...
    public boolean equals(Object obj) {
      if (obj == null) {
        ...
        return editor.fieldNameLb.getText().trim().equals(
            fieldNameLb.getText().trim());
      }
      return false;
    }

    /**
     * Calls {@link #onOccurrenceUpdate()} when a save action is enforce. This
     * method is call by {@link DetailView} when "Save Changes" button is
     * clicked.
     */
    ...
    /**
     * A text from the text box is consider to be changed if it is differed from
     * the original text and the input text is valid.
     * 
    ...
    /**
     * Sets CSS style to {@link #SELECTED_STYLE} if selected is true, and sets
     * it to {@link #NON_SELECTED_STYLE} otherwise.
     * 
     ...
     * 
     * @param editable true if this value field is editable.
     */
     ...
     * input text box then sets it text to display value text and selects all
     * the text in input text box.
     */
  ...
  private abstract class DetailItem extends TreeItem implements
     ...
     * 
     * A loading item is initially added for the expand button to appear. It is
     * deleted from the tree when this tree is loaded.
     * 
     * @param rootName the name of this tree node.
     */
    ...
    public void restoreOriginalValues() {
      if (loaded) {
        for (int i = 0; i < super.getChildCount(); i++) {
          FieldEditor editor = (FieldEditor) super.getChild(i).getWidget();
     ...
     * ediable.
     * 
     * Note: all fields in {@link FieldConstants#NON_EDITABLE_FIELDS} will not
     * be set to be editable.
     * 
    ...
    public void setFieldsEditable(boolean editable) {
      ...
      if (loaded) {
        for (int i = 0; i < getChildCount(); i++) {
          FieldEditor editor = (FieldEditor) getChild(i).getWidget();
          if (!FieldConstants.NON_EDITABLE_FIELDS.contains(editor.getName())) {
     ...
     * 
     * @param loaded true if this tree haven't yet initialized or it need to be
    ...
    /**
     * Adds the given {@link FieldEditor} if it is not yet in the tree. Resets
     * this field to the given editor value if it is already in the tree. An
     * editor is considered to be in the child items if there is an item with
     * same field name existed in its child items.
     * 
     * @param editor {@link FieldEditor}
     */
    ...
    /**
     * Loads all items from the {@link DetailView#currentOccurrence} that belong
     * to this tree.
     */
  ...
  private class ReviewersReviewed extends Composite {
    ...
    public ReviewersReviewed() {
      ...
      reviewerTable.setHeaders(REVIEWER_TABLE_HEAER, REVIEWER_TABLE_HEADER_CSS);
   ...
   * 
   */
  private class SmallMap extends Composite implements MarkerDragHandler,
    ...
    public SmallMap() {
      ...
      marker = new Marker(LatLng.newInstance(-91, -91), markerOptions);
      marker.addMarkerDragHandler(this);
      marker.addMarkerDragEndHandler(this);
    }
     ...
     * 
     * @param latitude double latitude of the marker
     * @param longitude double longitude of the marker
     */
    public void setMarker(LatLng latLng) {
      marker.setLatLng(latLng);
      Icon icon = marker.getIcon();
      icon.setImageURL(DEFAULT_MARKER_ICON);
      icon.setShadowURL(DEFAULT_MARKER_SHADOW_URL);
      map.clearOverlays();
    ...
    public void setMarkerDragEnabled(boolean enabled) {
      if (marker != null) {
        marker.setDraggingEnabled(enabled);
      }
    }

    /**
     * Initializes this map with small map control and set a fixed with to it.
     */
  ...
  private static final String DEFAULT_MARKER_ICON = Icon.DEFAULT_ICON
      .getImageURL();

  private static final String DEFAULT_MARKER_SHADOW_URL = Icon.DEFAULT_ICON
      .getShadowURL();
  ...
  /**
   * A {@link HorizontalPanel} widget to display links and buttons on the top if
   * this view.
   */
   ...
   * 
   * @param pageListener TODO
   * @param changeListener TODO
   */
  public DetailView(View parent, PageListener<Occurrence> pageListener,
    ...
    detailTree.addKeyUpHandler(new KeyUpHandler() {
      public void onKeyUp(KeyUpEvent event) {
        ...
        if (event.getNativeKeyCode() == KeyCodes.KEY_ENTER && editor != null) {
  ...
  /**
   * Resets currentOccurrence to the given occurrence then load Occurrence field
   * values. Also disable editable fields if current selected occurrence is not
  ...
  public void onOccurrenceSelected(Occurrence occurrence,
    ...
    if (currentOccurrence == null || currentOccurrence != occurrence) {
      mainSp.setPixelSize(Window.getClientWidth(), Window.getClientHeight()
          - mainSp.getAbsoluteTop());
      ...
      try {
        ...
        newLng = Double.parseDouble(currentOccurrence.getDecimalLongitude());
        updateMarkerPosition();
      } catch (Exception e) {
        smallMap.setCenter();
      }
  ...
  /**
   * Loads the selected OccDetailItem if it is not yet loaded when the state of
   * this tree is changed.
   */
  ...
  public void onPageLoaded(List<Occurrence> data, int pageNumber) {
    ...
    for (Occurrence o : data) {
      ...
      occLink.select(o == currentOccurrence || o.getId().toString().equals(id));
      // } else {
   ...
   * Deletes a comment.
   * 
   * @param comment {@link OccurrenceComments} to be deleted.
   */
  ...
  private String getIdFromHistoryToken(String historyToken) {
    try {
      return ApplicationView.getHistoryTokenParamValues(historyToken, "id")
          .get(0);
    } catch (Exception e) {
      return null;
    }
  ...
  private String getIsPrintViewFromHistoryToken(String historyToken) {
    try {
      return ApplicationView.getHistoryTokenParamValues(historyToken, "p").get(
          0);
    } catch (Exception e) {
      return null;
    }
  ...
  private void loadCoreItems() {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DATE_LAST_MODIFIED, toNoNullString(currentOccurrence
            .getDateLastModified())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.INSTITUTION_CODE, toNoNullString(currentOccurrence
            .getInstitutionCode())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.COLLECTION_CODE, toNoNullString(currentOccurrence
            .getCollectionCode())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.CATALOG_NUMBER,
        toNoNullString(currentOccurrence.getCatalogNumber())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.INFORMATION_WITHHELD, toNoNullString(currentOccurrence
            .getInformationWithheld())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.RELATED_INFORMATION, toNoNullString(currentOccurrence
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.SCIENTIFIC_NAME, toNoNullString(currentOccurrence
            .getScientificName())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.HIGHER_TAXON,
        toNoNullString(currentOccurrence.getHigherTaxon())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.INFRASPECIFIC_RANK, toNoNullString(currentOccurrence
            .getInfraspecificRank())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.INFRASPECIFIC_EPITHET, toNoNullString(currentOccurrence
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        ...
        toNoNullString(currentOccurrence.getAuthorYearOfScientificName())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.HIGHER_GEOGRAPHY, toNoNullString(currentOccurrence
            .getHigherGeography())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.ISLAND_GROUP,
        toNoNullString(currentOccurrence.getIslandGroup())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.STATE_PROVINCE,
        toNoNullString(currentOccurrence.getStateProvince())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
      ...
      public void onOccurrenceUpdated() {
        String value = input.getText();
        currentOccurrence.setAdjustedCoordinateUncertaintyInMeters(value);
      }
    });
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.MINIMUM_ELEVATION_IN_METERS,
        toNoNullString(currentOccurrence.getMinimumElevationInMeters())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        ...
        toNoNullString(currentOccurrence.getMaximumElevationInMeters())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.COLLECTING_METHOD, toNoNullString(currentOccurrence
            .getCollectingMethod())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.LATEST_DATE_COLLECTED, toNoNullString(currentOccurrence
            .getLatestDateCollected())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.DAY_OF_YEAR,
        toNoNullString(currentOccurrence.getDayOfYear())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.MONTH_COLLECTED, toNoNullString(currentOccurrence
            .getMonthCollected())) {
    ...
    coreItems.addFieldEditorItem(new FieldEditor(FieldConstants.DAY_COLLECTED,
        toNoNullString(currentOccurrence.getDayCollected())) {
  ...
  private void loadCuraItems() {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.CATALOG_NUMBER_NUMERIC, toNoNullString(currentOccurrence
            .getCatalogNumberNumeric())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.IDENTIFIED_BY,
        toNoNullString(currentOccurrence.getIdentifiedBy())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DATE_IDENTIFIED, toNoNullString(currentOccurrence
            .getDateIdentified())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.COLLECTOR_NUMBER, toNoNullString(currentOccurrence
            .getCollectorNumber())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.FIELD_NUMBER,
        toNoNullString(currentOccurrence.getFieldNumber())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VERBATIM_ELEVATION, toNoNullString(currentOccurrence
            .getVerbatimElevation())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.VERBATIM_DEPTH,
        toNoNullString(currentOccurrence.getVerbatimDepth())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.TYPE_STATUS,
        toNoNullString(currentOccurrence.getTypeStatus())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.OTHER_CATALOG_NUMBERS, toNoNullString(currentOccurrence
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.DISPOSITION,
        toNoNullString(currentOccurrence.getDisposition())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.FIELD_NOTES,
        toNoNullString(currentOccurrence.getFieldNotes())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(FieldConstants.PREPARATIONS,
        toNoNullString(currentOccurrence.getPreparations())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.GEN_BANK_NUMBER, toNoNullString(currentOccurrence
            .getGenBankNumber())) {
    ...
    curaItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.INDIVIDUAL_COUNT, toNoNullString(currentOccurrence
            .getIndividualCount())) {
  ...
  private void loadEnvItems() {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.DEMELEVATION,
        toNoNullString(currentOccurrence.getDemelevation())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.ETP_TOTAL2000,
        toNoNullString(currentOccurrence.getEtpTotal2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.ETP_TOTALFUTURE,
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.ETP_TOTAL1950,
        toNoNullString(currentOccurrence.getEtpTotal1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MAX_PREC2000,
        toNoNullString(currentOccurrence.getMaxPerc2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MAX_PRECFUTURE,
        toNoNullString(currentOccurrence.getMaxPercfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MAX_PREC1950,
        toNoNullString(currentOccurrence.getMaxPerc1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MAX_TEMP2000,
        toNoNullString(currentOccurrence.getMaxTemp2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MAX_TEMPFUTURE,
        toNoNullString(currentOccurrence.getMaxTempfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_PREC2000,
        toNoNullString(currentOccurrence.getMinPerc2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_PRECFUTURE,
        toNoNullString(currentOccurrence.getMinPercfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_PREC1950,
        toNoNullString(currentOccurrence.getMinPerc1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_TEMP2000,
        toNoNullString(currentOccurrence.getMinTemp2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_TEMPFUTURE,
        toNoNullString(currentOccurrence.getMinTempfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.MIN_TEMP1950,
        toNoNullString(currentOccurrence.getMinTemp1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MAR2000,
        toNoNullString(currentOccurrence.getRealMar2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MARFUTURE,
        toNoNullString(currentOccurrence.getRealMarfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MAR1950,
        toNoNullString(currentOccurrence.getRealMar1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MAT2000,
        toNoNullString(currentOccurrence.getRealMat2000())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MATFUTURE,
        toNoNullString(currentOccurrence.getRealMatfuture())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.REAL_MAT1950,
        toNoNullString(currentOccurrence.getRealMat1950())) {
    ...
    envItems.addFieldEditorItem(new FieldEditor(FieldConstants.WBYEARFUTURE,
        toNoNullString(currentOccurrence.getWbyearfuture())) {
  ...
  private void loadGeoItems() {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VERBATIM_COORDINATES, toNoNullString(currentOccurrence
            .getVerbatimCoordinates())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VERBATIM_LATITUDE, toNoNullString(currentOccurrence
            .getVerbatimLatitude())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VERBATIM_LONGITUDE, toNoNullString(currentOccurrence
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.GEOREFERENCE_PROTOCOL, toNoNullString(currentOccurrence
            .getGeoreferenceProtocol())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.GEOREFERENCE_SOURCES, toNoNullString(currentOccurrence
            .getGeoreferenceSources())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        ...
        toNoNullString(currentOccurrence.getGeoreferenceVerificationStatus())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.GEOREFERENCE_REMARKS, toNoNullString(currentOccurrence
            .getGeoreferenceRemarks())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(FieldConstants.FOOTPRINT_WKT,
        toNoNullString(currentOccurrence.getFootprintWkt())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.FOOTPRINT_SPATIAL_FIT, toNoNullString(currentOccurrence
            .getFootprintSpatialFit())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DEC_LAT_IN_WGS84, toNoNullString(currentOccurrence
            .getDecLatInWgs84())) {
    ...
    geoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DEC_LONG_IN_WGS84, toNoNullString(currentOccurrence
            .getDecLongInWgs84())) {
  ...
  /**
   * Loads the current occurrence require fields and value to the first item of
   * the detail tree;
   */
  private void loadRequiredItems() {
    ...
    /*
     * requiredItems.addFieldEditorItem(new FieldEditor(FieldConstants.OWNER,
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(FieldConstants.VALIDATED,
        toNoNullString(currentOccurrence.isValidated())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VALIDATION_ERROR, toNoNullString(currentOccurrence
            .getValidationError())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(FieldConstants.VETTABLE,
        toNoNullString(currentOccurrence.isVettable())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(FieldConstants.REVIEWED,
        toReviewedString(currentOccurrence.getReviewed())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.TAPIR_ACCESSIBLE, toNoNullString(currentOccurrence
            .isTapirAccessible())) {

      public void onOccurrenceUpdated() {
        String value = input.getText();
        currentOccurrence.setTapirAccessible(Boolean.parseBoolean(value));
      }
    });

    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.BASIS_OF_RECORD, toNoNullString(currentOccurrence
            .getBasisOfRecord())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.SPECIFIC_EPITHET, toNoNullString(currentOccurrence
            .getSpecificEpithet())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DECIMAL_LATITUDE, toNoNullString(currentOccurrence
            .getDecimalLatitude())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.DECIMAL_LONGITUDE, toNoNullString(currentOccurrence
            .getDecimalLongitude())) {
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.NOMENCLATURAL_CODE, toNoNullString(currentOccurrence
    ...
    requiredItems.addFieldEditorItem(new FieldEditor(
        ...
        toNoNullString(currentOccurrence.getCoordinateUncertaintyInMeters())) {
  ...
  private void loadTaxoItems() {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.VERBATIM_SPECIES, toNoNullString(currentOccurrence
            .getVerbatimSpecies())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_KINGDOM, toNoNullString(currentOccurrence
            .getAcceptedKingdom())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_PHYLUM, toNoNullString(currentOccurrence
            .getAcceptedPhylum())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(FieldConstants.ACCEPTED_CLASS,
        toNoNullString(currentOccurrence.getAcceptedClass())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(FieldConstants.ACCEPTED_ORDER,
        toNoNullString(currentOccurrence.getAcceptedOrder())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_SUBORDER, toNoNullString(currentOccurrence
            .getAcceptedSuborder())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_FAMILY, toNoNullString(currentOccurrence
            .getAcceptedFamily())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_SUBFAMILY, toNoNullString(currentOccurrence
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(FieldConstants.ACCEPTED_GENUS,
        toNoNullString(currentOccurrence.getAcceptedGenus())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_SUBGENUS, toNoNullString(currentOccurrence
            .getAcceptedSubgenus())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        ...
        toNoNullString(currentOccurrence.getAcceptedSpecificEpithet())) {
    ...
    taxoItems.addFieldEditorItem(new FieldEditor(
        FieldConstants.ACCEPTED_SPECIES, toNoNullString(currentOccurrence
            .getAcceptedSpecies())) {
  ...
  private void loadUserComment() {
    ...
    DataSwitch.get().fetch(query, new AsyncCallback<OccurrenceCommentQuery>() {
      public void onFailure(Throwable caught) {
        Window.alert(caught.getLocalizedMessage());
        GWT.log(caught.getLocalizedMessage(), caught);
  ...
  private void onHistoryChanged(String historyToken) {
    ...
    String id = historyState.getHistoryParameters(UrlParam.ID).toString();
    if (id == null
        || (currentOccurrence != null && currentOccurrence.getId().equals(id))) {
  ...
  /**
   * Restores all fields to the their original values and reset marker position.
   */
   ...
   * 
   * @param enabled true for enables
   */
  ...
  /**
   * Converts the given object as no null String. if it is null or empty return
   * "----".
   * 
   * @param obj to be convert to String
    ...
    DataSwitch.get().fetch(sessionId, query,
        new AsyncCallback<OccurrenceQuery>() {
          ...
          public void onSuccess(OccurrenceQuery result) {
            try {
              onOccurrenceSelected(result.getResults().iterator().next(), false);
            } catch (Exception e) {
              onFailure(e);
            }
          }
