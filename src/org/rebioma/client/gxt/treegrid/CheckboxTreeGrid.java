...
public class CheckboxTreeGrid<M extends ModelData> extends Grid<M> implements CheckProvider<M>{
	  ...
	  public class TreeNode {
		  protected Element joint, icon, check;
		    protected String id;		    
		    protected M m;
		    ...
		    public void setExpanded(boolean expand) {
		      CheckboxTreeGrid.this.setExpanded(m, expand);
		    }

		    public void setLeaf(boolean leaf) {
		      this.leaf = leaf;
		      CheckboxTreeGrid.this.refresh(m);
		    }		    
		  }
	  ...
	  protected String itemSelector = ".x-tree3-node";
		  ...
		  protected CheckboxTreeGridView treeGridView;
		  ...
		  private CheckCascade checkStyle = CheckCascade.CHILDREN;		  
		  ...
		  public CheckboxTreeGrid(TreeStore store, ColumnModel cm) {
		    ...
		    setView(new CheckboxTreeGridView());
		    disableTextSelection(true);
		    setSelectionModel(new CheckboxTreeGridSelectionModel<M>());
		  }
		   ...
		   * @return the view
		   */
		  public CheckboxTreeGridView getTreeView() {
		    return treeGridView;
		  }
		  ...
		  public void setExpanded(M model, boolean expand, boolean deep) {
		    ...
		    if (node != null) {
		      ...
		      CheckboxTreeGridEvent<M> tge = new CheckboxTreeGridEvent<M>(this, model);
		      tge.setTreeNode(node);
		  ...
		  public void setView(GridView view) {
		    assert view instanceof CheckboxTreeGridView : "The view for a CheckboxTreeGrid has to be an instance of CheckboxTreeGridView";
		    super.setView(view);
		    treeGridView = (CheckboxTreeGridView) view;
		  }
		  ...
		  protected ComponentEvent createComponentEvent(Event event) {
		    return new CheckboxTreeGridEvent(this, event);
		  }
		  ...
		  protected void onClick(GridEvent<M> e) {			  
		    ...
		    if (m != null) {
		      TreeNode node = findNode(m);
		      if (node != null) {
		    	  CheckboxTreeGridEvent<M> tge = new CheckboxTreeGridEvent<M>(this, e.getEvent(), m);
		    	  tge.setTreeNode(node);
		        ...
		        boolean isWithin = e.within(checkEl);
		        if (checkable && checkEl != null && isWithin) {		        	
		          onCheckClick(tge, node);
		        }
		  ...
		  protected void refresh(M model) {	  
		    ...
		    if (rendered && viewReady && node != null) {
		      ...
		      setChecked(model, node.checked && checkable);		      
		    }   
		  }
		  ...
		  protected void unregister(M m) {
		    ...
		    if (m != null && useKeyProvider != null && (node = findNode(m)) != null) {
		      ...
		      CheckboxTreeGridEvent<M> e = new CheckboxTreeGridEvent<M>(this, m);
		      e.setTreeNode(node);
	  ...
	  public void setChecked(M item, boolean checked) {		  
	      ...
	      CheckboxTreeGridEvent<M> evt = new CheckboxTreeGridEvent<M>(this, item);
	      evt.setChecked(checked);
	      evt.setTreeNode(node);
	...
	public void setCheckStyle(CheckCascade checkStyle) {
	  ...
	  @SuppressWarnings({"unchecked", "rawtypes"})
	  protected void onCheckClick(CheckboxTreeGridEvent<M> e, TreeNode node) {
	    e.stopEvent();
	    setChecked((M) e.getItem(), !node.checked);
	  }

/*	  
	  private void cleanCollapsed(M parent) {
		    List<M> list = treeStore.getChildren(parent, true);
		    for (M m : list) {
		      TreeNode node = findNode(m);
