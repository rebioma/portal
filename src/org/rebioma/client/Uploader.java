...
public class Uploader extends Composite implements IsUpdateable, IUploader, HasJsData {
  ...
  private void parseAjaxResponse(String responseTxt) {
    ...
    if (error != null) {
      ...
      int totalKB = 100;
      try{
    	  transferredKB = Integer.valueOf(Utils.getXmlNodeValue(doc, TAG_CURRENT_BYTES)) / 1024;
    	  totalKB = Integer.valueOf(Utils.getXmlNodeValue(doc, TAG_TOTAL_BYTES)) / 1024;
      }catch(Exception e){e.printStackTrace();}
      statusWidget.setFileName(basename+"/"+Utils.getXmlNodeValue(doc, "traitement"));
