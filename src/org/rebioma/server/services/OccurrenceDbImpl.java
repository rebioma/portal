...
public class OccurrenceDbImpl implements OccurrenceDb {
	...
	/**
	 * A simple helper class that parses a string filter into it's component
	 * parts (column, operator, value). Provides methods for testing the filter
	 * operation.
	 */
	public static class OccurrenceFilter extends QueryFilter {
		...
		public OccurrenceFilter(String col, Operator op, Object value)
		...
		public String getPropertyName(String property) {
			...
			// A private column doesn't exist, so we map it to public here:
					...
					|| property.equalsIgnoreCase("public")
			...
			} else if (property.equalsIgnoreCase("class_")
					|| property.equalsIgnoreCase("class")) {
				property = "class_";
				...
				property = "continent";
					...
					.equalsIgnoreCase("coordinateUncertaintyInMeters")) {
				...
				property = "georeferenceSources";
					...
					.equalsIgnoreCase("georeferenceVerificationStatus")) {
				...
				property = "infraspecificEpithet";
					...
					.equalsIgnoreCase("taxonRank"/* "infraspecificRank" */)) {
	...
	public static void main(String args[]) {
		...
		// OccurrenceDbImpl().checkForReviewedChanged(115267));

		// OccurrenceDb occurrenceDb = DBFactory.getOccurrenceDb();
		/*
		 * OccurrenceQuery query = new OccurrenceQuery(0,
		 * OccurrenceQuery.UNLIMITED); try { for (Integer id :
		 * occurrenceDb.findOccurrenceIdsByQuery(query, (User) null)) {
		 * occurrenceDb.checkForReviewedChanged(id); } } catch (Exception e) {
		 * // TODO Auto-generated catch block e.printStackTrace(); }
		 */
		// Set<AttributeValue> attributeValues = new
		// HashSet<OccurrenceDb.AttributeValue>();
	...
	TaxonomicReviewerDb taxonomicReviewerDb = DBFactory
			.getTaxonomicReviewerDb();
	...
	public int assignReviewer(String userEmail, String taxoFieldName,
		...
		try {
			...
			AttributeValue attributeValue = new AttributeValue(taxoFieldName,
					taxoFieldValue);
			if (user != null && attributeValue.isValid()) {
				// check to see whether this assignment is already existed
				if (!taxonomicReviewerDb.isAssignmentExisted(user.getId(),
						attributeValue.getAttribute(),
						attributeValue.getValue())) {
					TaxonomicReviewer taxoReviewer = new TaxonomicReviewer(
							user.getId(), attributeValue.getAttribute(),
							attributeValue.getValue(), isMarine, isTerrestrial);
					// persist the assignment to the database for future
					// occurrence
					// references.
					taxoReviewer = taxonomicReviewerDb.save(taxoReviewer);
					// find and assign all existence occurrences that match the
					// assigning
					// taxonomic field and value pair
					// List<Occurrence> occurrences =
					// findByAttributeValue(attributeValue);
					List<Integer> ids = findValidatedOccIdsByAttributeValue(
							attributeValue, isMarine, isTerrestrial);
					System.out.println("assigning reviewer to " + ids.size()
							+ " matches occurrences");
					...
					System.out.println("\ndone assigned " + ids.size()
							+ " occurrences to " + userEmail + " and "
							+ assignOccsCount + " was assigned");
				} else {
					List<RecordReview> recordReviews = recordReviewDb
							.getRecordReviewsByUser(user.getId());
					Map<Integer, Integer> recordIds = new HashMap<Integer, Integer>();
					for (RecordReview recordReview : recordReviews) {
						recordIds.put(recordReview.getOccurrenceId(),
								recordReview.getOccurrenceId());
					}
					// TaxonomicReviewer taxoReviewer = new
					// TaxonomicReviewer(user.getId(),
					// attributeValue.getAttribute(), attributeValue.getValue(),
					// isMarine, isTerrestrial);
					// taxoReviewer = taxonomicReviewerDb.save(taxoReviewer);
					List<Integer> ids = findValidatedOccIdsByAttributeValue(
							attributeValue, isMarine, isTerrestrial);
					List<Integer> newIds = new ArrayList<Integer>();
					for (Integer i : ids) {
						if (recordIds.get(i) == null)
							newIds.add(i);
					}
					System.out.println("assigning reviewer to " + newIds.size()
							+ " matches occurrences");
			...
			e.printStackTrace();
			throw new OccurrenceServiceException("unable to assign "
					+ taxoFieldName + " with value " + taxoFieldValue
					+ " for user" + userEmail);
		}
	}
	...
	public void attachClean(Session session, Occurrence instance) {
		...
		try {
			// ManagedSession.createNewSessionAndTransaction().lock(instance,
			// LockMode.NONE);
	...
	public void attachDirty(Set<Occurrence> instances, Traitement traitement,
		...
		try {
			...
			for (Occurrence instance : instances) {
				traitement.setTraitement("Attach dirty Occurrences ",
						cent * 1024, ++i * 1024);
				...
				// {WD
				if (!isSA)
					instance.setLastUpdated((new Timestamp(System
				...
				// }
				try {
					instance.setDecimalLatitude(instance.getDecimalLatitude()
							.replace(',', '.'));
					instance.setDecimalLongitude(instance.getDecimalLongitude()
							.replace(',', '.'));
				} catch (Exception e) {
				}
				...
				if (newOccurrence) {
					...
					if ((instance.isValidated() == null || instance
							.isValidated())
							&& assignAndReviewRecords(instance, date, session)) {
					...
					if (isSA) {
						if (clearReview) {
							List<RecordReview> recordReviews = recordReviewDb
									.findByProperty(instance.getId(), rcdrv);
							for (RecordReview recordReview : recordReviews) {
						...
						// List<RecordReview> recordReviews =
						// recordReviewDb.getRecordReviewsByOcc(instance.getId());
						if (instance.getNoAssignation() != null
								&& instance.getNoAssignation()) {
							continue;
						}
						List<RecordReview> recordReviews = recordReviewDb
								.findByProperty(instance.getId(), rcdrv);
		...
		try {
			...
			for (Occurrence instance : instances) {
				...
				if (newOccurrence) {
					...
					List<RecordReview> recordReviews = recordReviewDb
							.getRecordReviewsByOcc(instance.getId());
					for (RecordReview recordReview : recordReviews) {
						recordReview.setReviewed(null);
						recordReview.setReviewedDate(null);
						session.update(recordReview);
	...
	public boolean checkForReviewedChanged(int occurrenceId) {
		try {
			...
			Boolean newReviewed = RecordReviewUtil.isRecordReviewed(
					recordReviewDb.getRecordReviewsByOcc(occurrenceId),
					RecordReviewUtil.DEFAULT_PERCENT_CUT);
	...
	/**
	 * Deletes all {@link Occurrence} found by this query and belong to the
	 * given user, found by user id. First delete records with original query.
	 * If there is no record to delete with the first delete then replace
	 * original query where acceptedSpecies "=" to acceptedSpecies "like" and
	 * call delete again.
	 * 
	 * @see org.rebioma.server.services.OccurrenceDb#detele(OccurrenceQuery,
	 *      User)
	 */
	public int detele(OccurrenceQuery query, User user) {
		int deletedRecord = 0;
		if (user != null) {
			Set<OccurrenceFilter> searchFilters = QueryFilter.getFilters(
					query.getBaseFilters(), OccurrenceFilter.class);
			searchFilters.addAll(QueryFilter.getFilters(
					query.getSearchFilters(), OccurrenceFilter.class));
			deletedRecord = deleteByQuery(user, searchFilters,
					query.getResultFilter(), 1);
			if (deletedRecord == 0) {
			...
			for (AttributeValue attributeValue : attributeValues) {
				if (orCriterion == null) {
					...
					orCriterion = Restrictions.or(orCriterion,
							attributeValue.toCriterion());
				}
			}
			criteria.add(orCriterion);
			List<Occurrence> results = criteria.list();
			log.debug("find by attributeValue successful, result size: "
					+ results.size());
			// if (isFirstTransaction) {
			// HibernateUtil.commitCurrentTransaction();
			// }
	...
	public List<Occurrence> findByExample(Occurrence instance) {
		...
		try {
			...
			// HibernateUtil.beginTransaction(session);
			Session session = ManagedSession.createNewSessionAndTransaction();
			List<Occurrence> results = session
					.createCriteria("org.rebioma.client.bean.Occurrence")
	...
	public List<OccurrenceReview> getOccurrenceReviewsOf(int occurrenceId) {
		try {
			...
			Session session = ManagedSession.createNewSessionAndTransaction();
			List<RecordReview> recordReviews = recordReviewDb
					.getRecordReviewsByOcc(occurrenceId);
			...
			if (recordReviews != null) {
				for (RecordReview recordReview : recordReviews) {
					...
					occurrenceReviews.add(new OccurrenceReview(reviewer
							.getFirstName() + " " + reviewer.getLastName(),
							reviewer.getEmail(), occurrenceId, recordReview
									.getReviewed(), recordReview
									.getReviewedDate()));
				}
			}
			// if (isFirst) {
	...
	public String removeBadId(Set<Occurrence> instances, User loggedinUser) {
		...
		for (Occurrence o : instances) {
			...
			// remove the occurrence that has owner id does not match with owner
			// id or
			// owner email does not match with user email.

			if (o.getId() == null) {
				String createdTime = (new Timestamp(System.currentTimeMillis()))
						.toString();
				...
				String existenceOwnerEmail = existenceOccurrence == null ? null
						: existenceOccurrence.getOwnerEmail().trim();
				...
				} else if ((!existenceOwnerEmail.equalsIgnoreCase(ownerEmail) || !existenceOwnerEmail
						.equalsIgnoreCase(currentUserEmail))
						&& !sAdmin
						&& !((o.getSharedUsersCSV() + "")
								.contains(currentUserEmail) && o
								.getNoAssignation())) {
					if (toRemove.add(o)) {
						log.error("attach failed (" + o
								+ ") record is not belong to "
								+ currentUserEmail);
	...
	public void resetReviews() {
		try {
			Session session = ManagedSession.createNewSessionAndTransaction();
			session.createQuery("update Occurrence o set o.reviewed=null")
					.executeUpdate();
			ManagedSession.commitTransaction(session);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public void updateStability(Occurrence o, Boolean stability, Session sess) {
		try {
			// Query query =
			// sess.createQuery("update Occurrence o set o.stability=:stability where o.id=:ID");
			Query query = sess
					.createQuery("update Occurrence o set o.stability="
							+ stability + " where o.id=" + o.getId());
	...
	/**
	 * First update records with original query if there is no update, replace
	 * the original query acceptedSpecies = to acceptedspecies like query. If
	 * user is trying to vet all records restrict the update query to only
	 * updated validated records.
	 * 
	...
	public int update(OccurrenceQuery query, User user) {
		...
		filters.addAll(QueryFilter.getFilters(query.getSearchFilters(),
				OccurrenceFilter.class));
		Set<OccurrenceFilter> updates = QueryFilter.getFilters(
				query.getUpdates(), OccurrenceFilter.class);
		int updatedRecordCount = updateByQuery(user, filters, updates,
				query.getResultFilter(), 1);
	...
	public String validateOccurrences(Set<Occurrence> instances,
			User loggedinUser, Map<User, Set<Occurrence>> userOccurrencesMap) {
		StringBuilder improperIds = new StringBuilder("[");
		StringBuilder idNotinDb = new StringBuilder("[");
		StringBuilder multipleId = new StringBuilder("[");
		StringBuilder notYourRecords = new StringBuilder("[");
		...
		try {
			for (Occurrence o : instances) {
				...
				// remove the occurrence that has owner id does not match with
				// owner id
				// or
				// owner email does not match with user email.

				if (o.getId() == null) {
					String createdTime = (new Timestamp(
					...
					if (existenceOccurrence == null) {
					...
					} else if (!existenceOccurrence.getOwnerEmail()
							.equalsIgnoreCase(ownerEmail)
							|| !existenceOccurrence.getOwnerEmail()
						...
						if (testDuplicate.containsKey(id)) {
							...
							// mark record that are change which i need to
							// notified user
							List<RecordReview> recordReviews = recordReviewDb
									.getRecordReviewsByOcc(id);
							for (RecordReview recordReview : recordReviews) {
								int userId = recordReview.getUserId();
								List<TaxonomicReviewer> reviewers = taxonomicReviewerDb
										.getTaxonomicReviewers(recordReview
												.getUserId());
								for (TaxonomicReviewer reviewer : reviewers) {
									String taxoField = reviewer
											.getTaxonomicField();
									String taxoValue = reviewer
											.getTaxonomicValue();
									if (isTaxonomicMatch(new AttributeValue(
											taxoField, taxoValue), o)) {
										User user = userDb.findById(userId);
										Set<Occurrence> notifyingOccs = userOccurrencesMap
												.get(user);
										if (notifyingOccs == null) {
											notifyingOccs = new HashSet<Occurrence>();
											userOccurrencesMap.put(user,
													notifyingOccs);
										}
										notifyingOccs.add(o);
									}
	 ...
	 * 
	 * @param criteria
	 *            {@link Criteria} to be added with filters
	 * @param user
	 *            an id of a user in db.
	 * @param searchFilters
	 *            a filters that use for find Occurrences.
	 * @param resultFilter
	 *            the {@link ResultFilter} to determine what public filter is
	 *            added.
	 * @param tryCount
	 *            TODO
	...
	private Set<String> addCreterionByFilters(Criteria criteria, User user,
		...
		for (OccurrenceFilter filter : searchFilters) {
			...
			if (filter.column.equalsIgnoreCase(filter
				...
				String acceptedSpecies = filter
						.getPropertyName("acceptedSpecies");
				String verbatimSpecies = filter
						.getPropertyName("verbatimSpecies");
				String scientificName = filter
						.getPropertyName("scientificName");
				if (tryCount == 1) {
					acceptedSpeciesCri = Restrictions.ilike(acceptedSpecies,
							filter.getValue().toString(), MatchMode.START);
					verbatimSpeciesCri = Restrictions.ilike(verbatimSpecies,
							filter.getValue().toString(), MatchMode.START);
					scientificNameCri = Restrictions.ilike(scientificName,
							filter.getValue().toString(), MatchMode.START);
					queryFilter = acceptedSpecies + " like '"
							+ filter.getValue() + "%' or " + verbatimSpecies
							+ " like '" + filter.getValue() + "%' or "
							+ scientificName + " like '" + filter.getValue()
							+ "%'";
				} else if (tryCount == 2) {
					acceptedSpeciesCri = Restrictions.ilike(acceptedSpecies,
							filter.getValue().toString(), MatchMode.ANYWHERE);
					verbatimSpeciesCri = Restrictions.ilike(verbatimSpecies,
							filter.getValue().toString(), MatchMode.ANYWHERE);
					scientificNameCri = Restrictions.ilike(scientificName,
							filter.getValue().toString(), MatchMode.ANYWHERE);
				...
				criterion = Restrictions
						.or(scientificNameCri, Restrictions.or(
								acceptedSpeciesCri, verbatimSpeciesCri));
			} else {
				Operator op = filter.getOperator();
				String value;
				switch (op) {
				case CONTAIN:
					...
					criterion = Restrictions.ilike(filter.column, value,
							MatchMode.ANYWHERE);
					break;
				case EQUAL:
					criterion = null;
					if (filter.column.equals(filter.getPropertyName("sex"))) {
						...
						if (value.equalsIgnoreCase("Non-standardSpecimen")) {
							Object[] filters = getComplexCriterion(
									VALID_BASIS_OF_RECORDS, filter.column, "!=");
							criterion = (Criterion) filters[0];
							queryFilter = filters[1].toString();
						} else {
							criterion = Restrictions.eq(filter.column, value);
						...
						if (StringUtil.isType(Occurrence.class, filter.column,
								String.class))
							criterion = Restrictions.sqlRestriction(
									"upper({alias}."
											+ StringUtil.columnName(
													Occurrence.class,
													filter.column)
											+ ") = upper(?)",
									filter.getValue(), Hibernate.STRING);
						else
							criterion = Restrictions.eq(filter.column,
									filter.getValue());
					}
					break;
				case NOT_CONTAIN:
					value = (String) filter.getValue();
					criterion = Restrictions.not(Restrictions.ilike(
							filter.column, value, MatchMode.ANYWHERE));
					break;
				case NOT_EQUAL:
					criterion = null;
					if (filter.column.equals(filter.getPropertyName("sex"))) {
						...
						criterion = Restrictions.ne(filter.column,
								filter.getValue());
					}
					break;
				case START_WITH:
				...
				case NOT_START_WITH:
					value = (String) filter.getValue();
					criterion = Restrictions.not(Restrictions.ilike(
							filter.column, value, MatchMode.START));
					break;
				case LESS:
					criterion = Restrictions.lt(filter.column,
							filter.getValue());
					break;
				case GREATER:
					criterion = Restrictions.gt(filter.column,
							filter.getValue());
					break;
				case LESS_EQUAL:
					criterion = Restrictions.le(filter.column,
							filter.getValue());
					break;
				case GREATER_EQUAL:
					criterion = Restrictions.ge(filter.column,
							filter.getValue());
					break;
				case IN:
					if (filter.getValue() instanceof Collection<?>) {
						criterion = Restrictions.in(filter.column,
				...
				case IS_EMPTY:
					if (StringUtil.isString(Occurrence.class, filter.column)) {
						...
						criterion = Restrictions.or(
								Restrictions.isNull(filter.column),
				...
				case IS_NOT_EMPTY:
					criterion = Restrictions.and(
							Restrictions.isNotNull(filter.column),
		...
		Criterion publicCriterion = getPublicCriterion(user, resultFilter,
				isMyOccurrence, filtersString);
	...
	/**
	 * Vérifie si l'occurrence appartient à un trb et qu'elle sera révisée et ne
	 * sera pas assigné à d'autre trb
	 * 
	 * @param occ
	 * @param date
	 * @return
	 */
	private boolean assignAndReviewRecords(Occurrence occ, Date date,
			Session session) {
		...
		if (occ.getNoAssignation() != null && occ.getNoAssignation()) {
			for (TaxonomicReviewer taxonomicReviewer : taxonomiKRB) {
				if (noAssignation = occ.getOwner().equals(
						taxonomicReviewer.getUserId())) {
					session.saveOrUpdate(new RecordReview(taxonomicReviewer
							.getUserId(), occ.getId(), true, date));
					break;
				}
			}
		}
	...
	private void assignReviewRecords(Occurrence occ, Session session) {
		...
		for (TaxonomicReviewer taxonomicReviewer : taxonomiKRB) {
			...
			if (isTaxonomicMatch(attributeValue, occ)
					&& checkAcceptedspeciesLocation(taxonomicReviewer,
							occ.getAcceptedSpecies())) {
				RecordReview recordReview = new RecordReview(
						taxonomicReviewer.getUserId(), occ.getId(), null);
   ...
   * 
   */
	private boolean checkAcceptedspeciesLocation(TaxonomicReviewer trb,
			String acceptedSpecies) {
		Session session = ManagedSession.createNewSessionAndTransaction();
		int isM = trb.getIsMarine() ? 1 : 0;
		int isT = trb.getIsTerrestrial() ? 1 : 0;
		Query query = session
				.createQuery("select id from Taxonomy t where t.acceptedSpecies "
						+ "='"
						+ acceptedSpecies
						+ "' and (t.isMarine = "
						+ isM
	...
	/**
	 * Create {@link RecordReview} for each of the occurrence that not yet
	 * assign to user with user id = {@link TaxonomicReviewer#getUserId()}.
	 * 
	 * @param taxonomicReviewer
	 */
	private Set<Occurrence> assignReviewToOccurrence(
			TaxonomicReviewer taxonomicReviewer, List<Occurrence> occurrences) {
		Set<Occurrence> assignedOccs = new HashSet<Occurrence>();
		if (occurrences != null) {
			for (Occurrence occurrence : occurrences) {
				int occurrenceId = occurrence.getId();
				// only create record review for occurrence that have not yet
				// assign to
				// this user
				RecordReview recordReview = new RecordReview(
						taxonomicReviewer.getUserId(), occurrenceId, null);
				recordReview = recordReviewDb.save(recordReview);
				if (recordReview != null) {
	...
	private String createDeleteQuery(Criteria criteria,
			Set<OccurrenceFilter> searchFilters, User user,
			ResultFilter resultFilter, int tryCount) {
		return "delete Occurrence"
				+ createWhereClause(criteria, user, searchFilters,
	 ...
	 * after userId to ensure searching user id in sharedUsersCSV return the
	 * correct result (i.e 18 and 8 are there same in like query without extra
	 * spaces).
	 * 
	 * @param csvUserIdsList
	 *            user list In CSV;
	 * @param userEmail
	 ...
	 */
	private String createUpdatedSharedUsersCSVSQL(String csvUserIdsList,
			String userEmail) {
		...
		for (String email : usersList) {
			...
			sb.append(" CASE WHEN sharedUsersCSV like '%" + email
					+ "%' then '' else '" + email + ",' END, ");
		}
		if (sb.length() > 0) {
	 ...
	 * user id. If user is trying to vet all records restrict the update query
	 * to only updated validated records.
	 * 
	 * @param criteria
	 *            {@link Criteria} to be added {@link Criterion} filters.
	 * @param searchFilters
	 *            filters use to find to be updated occurrences
	 * @param updateFilters
	 *            filters contain updated fields.
	 * @param user
	 *            id of this current user;
	 * @param resultFilter
	 *            the {@link ResultFilter} to determine what public filter is
	 *            added.
	 * @param tryCount
	 ...
	 */
	private String createUpdateQuery(Criteria criteria,
			Set<OccurrenceFilter> searchFilters,
			Set<OccurrenceFilter> updateFilters, User user,
			ResultFilter resultFilter, int tryCount) {
		...
		for (OccurrenceFilter filter : updateFilters) {
			...
			if (filter.column.equals(filter.getPropertyName("vetted"))
					&& filter.value.equals(true)) {
				addVettedRetriction = true;
			}

			if (filter.column.equals(filter.getPropertyName("sharedUsers"))) {
				sb.append(createUpdatedSharedUsersCSVSQL(
						filter.value.toString(), user.getEmail()));
			} else if (filter.column.equals(filter
					.getPropertyName("unsharedUsers"))) {
		...
		// removed public = false in searchFilters if this is My Occurrences
		// only
		...
		sb.append(createWhereClause(criteria, user, searchFilters,
				resultFilter, tryCount));
		return sb.toString();
	}
	 ...
	 * 
	 * @param criteria
	 *            {@link Criteria} contains the query table object.
	 * @param user
	 *            id of this current user;
	 * @param searchFilters
	 *            filters use to find to be updated occurrences
	...
	private String createWhereClause(Criteria criteria, User user,
			Set<OccurrenceFilter> searchFilters, ResultFilter resultFilter,
			int tryCount) {
		// Set<Filter> privateFilters = isMyOccurrenceOnly(userId,
		// searchFilters);
		Set<String> queryFilters = this.addCreterionByFilters(criteria, user,
				searchFilters, resultFilter, tryCount);
		StringBuilder sb = new StringBuilder(" where (ownerEmail='"
				+ user.getEmail() + "' or (sharedusers like '%"
				+ user.getEmail() + "%' and noassignation = true))");
	 ...
	 * 
	 * @param user
	 *            user that own deleted records.
	 * @param searchFilters
	 *            {@link OccurrenceFilter} use to search for deleted record
	 * @param resultFilter
	 *            the {@link ResultFilter} to determine what public filter is
	 *            added.
	 * @param tryCount
	...
	private int deleteByQuery(User user, Set<OccurrenceFilter> searchFilters,
		...
		try {
			String deleteQuery = createDeleteQuery(
					session.createCriteria(Occurrence.class), searchFilters,
					user, resultFilter, tryCount);
	...
	private List<Integer> findIds(OccurrenceQuery query,
		...
		try {
			...
			log.info("find filters: "
					+ addCreterionByFilters(criteria, user, filters,
							query.getResultFilter(), tryCount));
			if (userReviewFilter != null) {
				filters.remove(idsFilter);
				filters.add(userReviewFilter);
			}
	...
	private List<Integer> findOccIdsByAttributeValue(
			AttributeValue attributeValue) {
		Session session = ManagedSession.createNewSessionAndTransaction();
		Query query = session
				.createQuery("select id from Occurrence o where o."
						+ attributeValue.getAttribute() + "='"
						+ attributeValue.getValue() + "'");
		List<Integer> ids = query.list();
		ManagedSession.commitTransaction(session);
	...
	private List<Integer> findValidatedOccIdsByAttributeValue(
			AttributeValue attributeValue) {
		Session session = ManagedSession.createNewSessionAndTransaction();
		Query query = session
				.createQuery("select id from Occurrence o where o."
						+ attributeValue.getAttribute() + "='"
						+ attributeValue.getValue() + "' and o.validated=true");
		List<Integer> ids = query.list();
		ManagedSession.commitTransaction(session);
		return ids;
	}
	...
	// {WD}
	private List<Integer> findValidatedOccIdsByAttributeValue(
			AttributeValue attributeValue, boolean isMarrine,
		...
		try {
			...
			ResultSet rst = st
					.executeQuery("select o.id from Occurrence o left join taxonomy t on (upper(o.acceptedspecies) = upper(t.acceptedspecies)) where upper(o."
							+ attributeValue.getAttribute()
							+ ")=upper('"
							+ attributeValue.getValue()
							+ "') and o.validated=true and (t.ismarine = "
							+ isM
							+ " or t.isterrestrial = "
							+ isT
							+ ") and o.noassignation is not true");
	...
	private Object[] getComplexCriterion(String values[], String fieldName,
		...
		for (String value : values) {
			if (criterion == null) {
				...
				queryFilter += filterOperator + fieldName + operator + "'"
						+ value + "'";
				if (operator.equals("!=")) {
	...
	private FilterBuilder getPublicFilter(User user, ResultFilter resultFilter,
		...
		String publicCol = getOccurrencePropertyName("public").toLowerCase();
		String ownerCol = getOccurrencePropertyName("ownerEmail").toLowerCase();
		FilterBuilder filterBuilder = null;
		switch (resultFilter) {
		case PUBLIC:
	...
	/**
	 * Gets A criterion that constrain the query to the given
	 * {@link ResultFilter} .
	 * 
	 * @param user
	 *            logged in user id
	 ...
	 * 
	 *            sharedUsersCSV format: space userid space, space userid space,
	 *            ..., .There is and comma in the end of the id list to rule out
	 *            end of the list corner case when a user id is removed from the
	 *            list. A space before and after userId to ensure searching user
	 *            id in sharedUsersCSV return the correct result (i.e 18 and 8
	 *            are there same in like query without extra spaces).
	 * @param isMyOccurrence
	 *            TODO
	 * @param queryFilters
	 *            TODO
	 * 
	...
	private Criterion getPublicCriterion(User user, ResultFilter resultFilter,
		...
		case PRIVATE:
			if (sAdmin) {
				...
				queryFilters.add(publicCol + "=false and " + ownerCol + "='"
						+ user.getEmail() + "'");
			}
			break;
		case BOTH:
			if (sAdmin) {
				...
				queryFilters.add(publicCri.toString() + " or "
						+ privateCri.toString());
			} else {
				Criterion publicCri = Restrictions.eq(publicCol, true);
				Criterion privateCri = Restrictions.and(
						Restrictions.eq(publicCol, false),
						Restrictions.eq(ownerCol,
								user == null ? null : user.getEmail()));
				String publicQ = publicCri.toString();
				String privateQ = publicCol + "=false and " + ownerCol + "='"
						+ user.getEmail() + "'";
				if (!isMyOccurrence) {
					privateCri = Restrictions.or(
							privateCri,
							Restrictions.ilike("sharedUsersCSV",
									user.getEmail(), MatchMode.ANYWHERE));
					criterion = Restrictions.or(publicCri, privateCri);
					queryFilters
							.add(publicQ + " or " + privateQ + " or "
									+ "sharedUsersCSV like '%"
									+ user.getEmail() + "%'");
				} else {
					criterion = Restrictions.or(publicCri, privateCri);
	...
	private boolean isTaxonomicChanged(AttributeValue attributeValue,
			Occurrence occA, Occurrence occB) {
		String propertyName = StringUtil.capFirstLetter(attributeValue
				.getAttribute());
		...
		try {
			...
			if (valueA != null) {
				if (valueB != null) {
					return !valueA.toString().equalsIgnoreCase(
							valueB.toString());
				} else {
					return true;
				}
	...
	private boolean isTaxonomicMatch(AttributeValue attributeValue,
	...
	private void resetOccurrenceReviewedStatus(int id) {
		Session session = ManagedSession.createNewSessionAndTransaction();
		Query query = session
				.createQuery("update Occurrence set reviewed=NULL where id="
						+ id);
		query.executeUpdate();
		ManagedSession.commitTransaction(session);
	}
	 ...
	 * 
	 * @param user
	 *            owner id for this update
	 * @param searchFilters
	 *            a filters that use for find Occurrences.
	 * @param updateFilters
	 *            a filters that use for updating {@link Occurrence}.
	 * @param resultFilter
	 *            the {@link ResultFilter} to determine what public filter is
	 *            added.
	 * @param tryCount
	...
	private int updateByQuery(User user, Set<OccurrenceFilter> searchFilters,
			Set<OccurrenceFilter> updateFilters, ResultFilter resultFilter,
			int tryCount) {
		...
		try {
			String updateQuery = createUpdateQuery(
					session.createCriteria(Occurrence.class), searchFilters,
					updateFilters, user, resultFilter, tryCount);
			log.info(updateQuery);
			int updatedRecords = session.createQuery(updateQuery)
					.executeUpdate();
	...
	public void resetRecordReview(Occurrence occurrence, boolean isStable,
		...
		occurrence.setLastUpdated((new Timestamp(System.currentTimeMillis()))
				.toString());
		occurrence.setReviewed(null);
		occurrence.setStability(isStable);
