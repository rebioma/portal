...
public class ValidationServiceImpl implements ValidationService {
	...
	private Polygon polygonTerrestrial;
	...
	private Polygon loadPolygon(String file) {
		...
		try {
			...
			while(sc.hasNext()) {
				...
				dY.add((int) (sc.nextDouble()*1000000));
				dX.add((int) (sc.nextDouble()*1000000));
			}
			
			int[] x = new int[dX.size()];

			int[] y = new int[dX.size()];
			for (int i = 0; i < y.length; i++) {
				x[i]=dX.get(i);
				y[i]=dY.get(i);
			}
		    
		    return new Polygon(x, y, dX.size());
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}finally{
	...
	public void loadPolygon() {
		polygonMarine = loadPolygon("mada_m.plg");
		polygonTerrestrial = loadPolygon("mada_t.plg");
	}
	
	public boolean isTerrestrial(String x, String y) {
		boolean isTerrestrial = true;
		try {
			Double X = Double.parseDouble(x);
			Double Y = Double.parseDouble(y);
			isTerrestrial = polygonTerrestrial.contains((int) (X*1000000), (int) (Y*1000000));
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean isMarine(String x, String y) {
		boolean isMarine = true;
		try {
			Double X = Double.valueOf(x);
			Double Y = Double.valueOf(y);
			isMarine = !polygonMarine.contains((int) (X*1000000), (int) (Y*1000000));
		}catch (Exception e) {
			e.printStackTrace();
		}
		return isMarine;
	...
	protected void validateTaxonomy(Occurrence occurrence) {
		...
		if (c != null) {
			...
			if (ok.equals("OK")){	
				...
				if(!(c.getIsMarine().equals("1") && c.getIsTerrestrial().equals("1"))) {
					
					if(c.getIsMarine().equals("1") && !isMarine(occurrence.getDecimalLatitude(), occurrence.getDecimalLongitude())) 
						updateValidationErrors(occurrence, "Marine data with wrong coordinate");
					if (c.getIsTerrestrial().equals("1") && !isTerrestrial(occurrence.getDecimalLatitude(), occurrence.getDecimalLongitude())) 
						updateValidationErrors(occurrence, "Terrestrial data with wrong coordinate");
				}
